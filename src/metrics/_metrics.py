...
def get_normalized_roughness_tv(l, d, i, m, J, theta, T, max_global_value, min_global_value):
    ...
    #print(J)
    #print(J(theta + s[1] * d[1]))
    #print(J(theta + s[2] * d[1]))
...
def igsd_from_scan(scan2D_result, num_samples=200):
    """
    Computes the Inverse of the Geometric Standard Deviation (IGSD) from a 2D scan result.

    Parameters:
    - scan2D_result: A 2D scan result containing values
    - num_samples: The number of samples to draw for computing IGSD

    Returns:
    - igsd_param_0: IGSD for parameter gamma
    - igsd_param_1: IGSD for parameter beta
    """

    def sample_expectations(scan2D_result, param_index, num_samples):
        """
        Draws samples of expectation values for a given parameter index from scan results.
        """
        sample_values = []

        for _ in range(num_samples):
            # Draw random indices for sampling within the scan result grid
            random_index_x = np.random.randint(0, scan2D_result.values.shape[0])
            random_index_y = np.random.randint(0, scan2D_result.values.shape[1])

            # Extract the value for the selected parameter
            if param_index == 0:
                value = scan2D_result.values[random_index_x, random_index_y]
            elif param_index == 1:
                value = scan2D_result.values[random_index_y, random_index_x]
            else:
                raise ValueError("Invalid parameter index. Must be 0 or 1.")

            sample_values.append(value)

        return sample_values

    # Draw samples for parameter 0
    expectations_param_0 = sample_expectations(scan2D_result, 0, num_samples)

    # Draw samples for parameter 1
    expectations_param_1 = sample_expectations(scan2D_result, 1, num_samples)

    # Calculate the geometric mean and geometric standard deviation for each set of samples
    def geometric_std_dev(sample_values):
        std_dev = np.std(sample_values)
        return 1/std_dev

    std_dev_param_0 = geometric_std_dev(expectations_param_0)
    std_dev_param_1 = geometric_std_dev(expectations_param_1)

    # Calculate the IGSD for each parameter
